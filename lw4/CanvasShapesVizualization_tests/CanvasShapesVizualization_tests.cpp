...
#include "../CanvasShapesVizualization/CTriangle.h"
#include "../CanvasShapesVizualization/CCircle.h"
#include "../CanvasShapesVizualization/CLineSegment.h"
#include "../CanvasShapesVizualization/CRectangle.h"
#include "../CanvasShapesVizualization/CommandHandler.h"

#include "../../fakeit.hpp"
using namespace fakeit;


void DefineMockCanvasActions(Mock<ICanvas>& mockCanvas, std::stringstream& output)
{
	When(Method(mockCanvas, DrawLine)).AlwaysDo([&output](CPoint start, CPoint end, uint32_t color)->void{
		output << std::format("Drawing line from ({}, {}) to ({}, {}) with color {}\n", 
		start.GetCoordinateX(), start.GetCoordinateY(), start.GetCoordinateX(), start.GetCoordinateY(), color);
		});
	When(Method(mockCanvas, FillPolygon)).AlwaysDo([&output](std::vector<CPoint> const& points, uint32_t fillColor)->void {
		output << std::format("Drawing figure with color {} with points:\n", fillColor);
		for (auto& point : points)
		{
			output << std::format("({}, {})\n", point.GetCoordinateX(), point.GetCoordinateY());
		}
		});
	When(Method(mockCanvas, DrawCircle)).AlwaysDo([&output](CPoint center, double radius, uint32_t lineColor)->void {
		output << std::format("Draw circle with center ({}, {}) and radius {}, with color {}\n",
		center.GetCoordinateX(), center.GetCoordinateY(), radius, lineColor);
		});
	When(Method(mockCanvas, FillCircle)).AlwaysDo([&output](CPoint center, double radius, uint32_t fillColor)->void {
		output << std::format("Fill circle with center ({}, {}) and radius {}, with color {}\n",
		center.GetCoordinateX(), center.GetCoordinateY(), radius, fillColor);
		});
}
...
TEST_CASE("Create CTriangle")
{
	...
	CHECK_THROWS_AS(CTriangle(CPoint(1, 2), CPoint(2, 4), CPoint(4, 8), 122122, 211211), std::runtime_error);
}
...
TEST_CASE("Handle not complete color and print results")
{
...
TEST_CASE("Draw CRectangle")
{
	CRectangle rect(CPoint(2, 3), CPoint(1, 4), 242300, 12421);
	Mock<ICanvas> mockCanvas;
	std::stringstream output;
	DefineMockCanvasActions(mockCanvas, output);
	rect.Draw(mockCanvas.get());

	CHECK(output.str() == "Drawing figure with color 242300 with points:\n(1, 3)\n(1, 4)\n(2, 4)\n(2, 3)\nDrawing line from (1, 3) to (1, 3) with color 12421\nDrawing line from (2, 3) to (2, 3) with color 12421\nDrawing line from (2, 4) to (2, 4) with color 12421\nDrawing line from (1, 4) to (1, 4) with color 12421\n");
}

TEST_CASE("Draw CLineSegment")
{
	CLineSegment line(CPoint(1, 2), CPoint(2, 4), 0xffffff);
	Mock<ICanvas> mockCanvas;
	std::stringstream output;
	DefineMockCanvasActions(mockCanvas, output);
	line.Draw(mockCanvas.get());

	CHECK(output.str() == "Drawing line from (1, 2) to (1, 2) with color 16777215\n");
}

TEST_CASE("Draw Triangle")
{
	CTriangle triangle(CPoint(1, 2), CPoint(2, 4), CPoint(3, 10), 0xff00ff, 0xff00ff);
	Mock<ICanvas> mockCanvas;
	std::stringstream output;
	DefineMockCanvasActions(mockCanvas, output);
	triangle.Draw(mockCanvas.get());

	CHECK(output.str() == "Drawing figure with color 16711935 with points:\n(1, 2)\n(2, 4)\n(3, 10)\nDrawing line from (1, 2) to (1, 2) with color 16711935\nDrawing line from (1, 2) to (1, 2) with color 16711935\nDrawing line from (3, 10) to (3, 10) with color 16711935\n");
}

TEST_CASE("Draw CCircle")
{
	CCircle circle(CPoint(100, 100), 229.321, 0xffffff, 0x000000);
	Mock<ICanvas> mockCanvas;
	std::stringstream output;
	DefineMockCanvasActions(mockCanvas, output);
	circle.Draw(mockCanvas.get());

	CHECK(output.str() == "Draw circle with center (100, 100) and radius 229.321, with color 0\nFill circle with center (100, 100) and radius 229.321, with color 16777215\n");
}